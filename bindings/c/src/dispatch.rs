...
use flare_im_core_sdk::model::message_elem::{elem_to_message_content, Elem};
...
fn json_vec_message(v: &serde_json::Value, key: &str) -> Result<Vec<IMMessage>, i32> {
    let arr = v.get(key).and_then(|x| x.as_array()).ok_or(FLARE_ERR_INVALID_PARAM)?;
    let mut out = Vec::with_capacity(arr.len());
    for e in arr {
...
pub extern "C" fn flare_message_dispatch_json(
    ...
    macro_rules! bad_param {
        () => {{
            return_error(&ctx, FLARE_ERR_INVALID_PARAM, "missing or invalid JSON field");
    ...
    match op.as_str() {
        "get" => {
            ...
            execute_async(instance, ctx, async move {
...
pub extern "C" fn flare_message_build_json(
    ...
    match op.as_str() {
        ...
        "create_thread_reply" => {
            ...
            execute_async(instance, ctx, async move {
                let b = client.message_build()?;
                b.create_thread_reply(&conversation_id, &thread_id, &text).await
            }, |m| to_json_string(&m));
        }
        "create_forward" => {
            let conversation_id = cid!();
            let merge = json_bool(&req, "merge").unwrap_or(false);
            let title = match json_str(&req, "forward_title").or_else(|_| json_str(&req, "title")) {
                Ok(s) => s.to_string(),
                Err(()) => bad_param!(),
            };
            ...
            execute_async(instance, ctx, async move {
                let b = client.message_build()?;
                b.create_forward(&conversation_id, merge, &title, sources).await
            }, |m| to_json_string(&m));
        }
        "create_image" => {
            let conversation_id = cid!();
        ...
        "create_sticker" => {
            ...
            let package_id = req.get("package_id").and_then(|x| x.as_str()).map(|s| s.to_string());
            let url = req.get("url").and_then(|x| x.as_str()).map(|s| s.to_string());
            let width = req.get("width").and_then(|x| x.as_i64()).map(|i| i as i32);
            let height = req.get("height").and_then(|x| x.as_i64()).map(|i| i as i32);
            let sticker_format = req.get("sticker_format").and_then(|x| x.as_str()).map(|s| s.to_string());
            execute_async(instance, ctx, async move {
                let b = client.message_build()?;
                b.create_sticker(
                    &conversation_id,
        ...
        "create_link_card" => {
            ...
            let title = req.get("title").and_then(|x| x.as_str()).map(|s| s.to_string());
            let description = req
                .get("description")
                .and_then(|x| x.as_str())
                .map(|s| s.to_string());
        ...
        "create_card" => {
            ...
            let subtitle = req
                ...
                .and_then(|x| x.as_str())
                .map(|s| s.to_string());
            let avatar = req.get("avatar").and_then(|x| x.as_str()).map(|s| s.to_string());
            execute_async(instance, ctx, async move {
                let b = client.message_build()?;
                b.create_card(
                    &conversation_id,
        ...
        "create_task" => {
            ...
            let status = req.get("status").and_then(|x| x.as_str()).map(|s| s.to_string());
            let participant_user_ids = req
                .get("participant_user_ids")
                .and_then(|x| serde_json::from_value(x.clone()).ok());
            execute_async(instance, ctx, async move {
