...
> 设计记录（work as-built）。核心能力已落地：core `upsert_user_profiles` + 测（SDK 绿）、契约路由（dispatch/uniffi/wasm）、五端 SDK 接线并各自构建通过、web 端到端实测。本文保留设计语义与决策，供后续演进（尤其 phase-2）参考。
