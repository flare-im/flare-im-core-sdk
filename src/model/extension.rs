...
pub trait ExtensionCache: Send + Sync {
    ...
    /// 保存会话扩展信息（到缓存）
    async fn save_session_extension(&self, session_id: &str, extension: &SessionExtension) -> Result<()>;
}
...
mod tests {
    ...
    fn test_message_extension() {
        ...
        assert_eq!(ext.sender_avatar.as_deref(), Some("https://example.com/avatar.jpg"));
