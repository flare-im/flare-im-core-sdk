...
pub use adapter::{DefaultPlatformAdapter, PlatformAdapter};
pub use capabilities::{NetworkType, PlatformCapabilities, StorageType};
...
impl PlatformConfig {
    ...
    pub fn for_platform(platform: Platform) -> Self {
        let (storage_type, network_type, supports_background, supports_push, memory_limit_mb) =
            match platform {
                Platform::Web => (
    ...
    pub fn supports_websocket(&self) -> bool {
        matches!(
            self.network_type,
            NetworkType::WebSocket | NetworkType::Both
        )
    }
}
