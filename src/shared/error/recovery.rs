...
impl RetryStrategy {
    ...
    pub fn should_retry(&self, error: &SDKError, attempt: u32) -> bool {
        ...
        match self {
            ...
            RetryStrategy::ExponentialBackoffWithJitter { max_attempts, .. } => {
        ...
        loop {
            match operation().await {
                ...
                Err(e) => {
                    ...
                    // 优先使用错误自身的重试策略（如果可用）
                    let should_retry = if e.is_retryable() {
                        let max_retries = e.max_retries();
                        attempt <= max_retries
                    } else {
                        strategy.should_retry(&e, attempt)
                    };
                    ...
                    if should_retry {
                        ...
                        let delay = e
                            ...
                            .or_else(|| strategy.calculate_delay(attempt));
