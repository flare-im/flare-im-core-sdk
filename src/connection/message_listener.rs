...
use crate::handler::MessageFrameHandler;
use crate::service::SyncService;
use crate::event::{Event, EventBus, ConnectionEvent};
...
impl MessageListener for SDKMessageListener {
    async fn on_message(&self, frame: &Frame) -> Result<Option<Frame>> {
        ...
        if is_custom_command || has_request_id_meta {
            ...
            if let Err(e) = self.sync_service.handle_response(frame.clone()).await {
                tracing::error!(
                    error = %e,
                    message_id = %frame.message_id,
    ...
    async fn on_connect(&self) -> Result<()> {
        ...
        debug!(
            "Client automatically sent CONNECT message with negotiation info"
        );
    ...
    async fn on_disconnect(&self, reason: Option<&str>) -> Result<()> {
        if let Some(reason) = reason {
            ...
            if reason.contains("设备冲突") || reason.contains("被踢") || reason.contains("device conflict") {
                tracing::error!(
                    reason = %reason,
        ...
        // 发布断开事件到事件总线
        self.event_bus.publish(Event::Connection(ConnectionEvent::Disconnected));
    ...
    async fn on_error(&self, error: &str) -> Result<()> {
        ...
        // 发布错误事件到事件总线
        self.event_bus.publish(Event::Connection(ConnectionEvent::Error(error.to_string())));
