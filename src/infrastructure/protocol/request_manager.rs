...
impl RequestManager {
    ...
    pub fn with_timeout(default_timeout: u64) -> Self {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                // 清理超时请求
                requests.retain(|_, req| now.duration_since(req.created_at).as_secs() < timeout);
    ...
    pub async fn cleanup_timeout_requests(&self, timeout: Option<u64>) {
        ...
        pending.retain(|_, req| now.duration_since(req.created_at).as_secs() < timeout_secs);
...
mod tests {
    ...
    use flare_core::common::protocol::{Command, Frame, Reliability};
    ...
    async fn test_request_manager() {
        ...
        // 创建请求
        let (request_id, receiver) = manager.create_request().await;
        ...
        // 完成请求
        manager
            .complete_request(&request_id, response.clone())
            .await
            .unwrap();
