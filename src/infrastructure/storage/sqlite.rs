...
use crate::infrastructure::storage::storage_trait::{
...
impl crate::infrastructure::storage::storage_trait::StorageSyncBounds for SqliteStorage {}
...
impl SqliteStorage {
    ...
    pub async fn new<P: AsRef<Path>>(db_path: P) -> Result<Self> {
        ...
        // 如果不是内存数据库，确保父目录存在
        if db_path_ref.to_string_lossy() != ":memory:" {
            ...
            // 将路径转换为绝对路径，确保 SQLite 可以正确访问
            let absolute_path = if db_path_ref.is_absolute() {
            ...
            // sqlx 的 SQLite 连接字符串格式：sqlite://<path> 或 sqlite:<path>
            // 对于文件路径，使用 sqlite:// 前缀，并确保路径是绝对路径
            // 如果文件不存在，先创建父目录，然后让 SQLite 创建数据库文件
            let canonical_path = if absolute_path.exists() {
                ...
                if let Some(parent) = absolute_path.parent() {
                    if !parent.exists() {
                        ...
                        info!(parent = %parent.display(), "Created parent directory for database");
                    }
                    // 验证父目录是否可写
                    let test_file = parent.join(".flare_im_test_write");
                    if let Err(e) = std::fs::write(&test_file, b"test") {
                        error!(parent = %parent.display(), error = %e, "Parent directory is not writable");
                ...
                // 使用绝对路径（不进行 canonicalize，因为文件还不存在）
                absolute_path.clone()
            };

            // sqlx 的 SQLite 连接字符串格式：
            // - 内存数据库：sqlite::memory:
            // - 文件数据库：sqlite:<path>（最简单格式，sqlx 会自动处理）
            // 直接使用绝对路径，sqlx 会自动识别
            let db_url = if cfg!(windows) {
                ...
                // Unix: 直接使用路径
                format!("sqlite:{}", canonical_path.to_string_lossy())
            };
            info!(db_path = %canonical_path.display(), db_url = %db_url, "Creating SQLite storage");

            let pool = Self::create_pool(&db_url).await?;
            let storage = Self { pool };
            storage.init_schema().await?;
            info!(db_path = %absolute_path.display(), "SQLite storage initialized");
            return Ok(storage);
        }
    ...
    /// 创建 SQLite 连接池（内部方法）
    async fn create_pool(db_url: &str) -> Result<SqlitePool> {
        // 根据平台调整连接池大小（使用优化的配置）
        use crate::shared::platform::{Platform, get_platform};
        let platform = get_platform();
        let (min_connections, max_connections) = match platform {
            Platform::Web => (1, 5),      // Web 端（虽然不会到达这里，但保留）
            Platform::Desktop => (5, 20), // 桌面端可以支持更多连接
            Platform::Android | Platform::IOS | Platform::HarmonyOS => (2, 10), // 移动端适中的连接数
        };

        // 对于文件数据库，使用 create_if_missing 选项
        ...
        // 如果不是内存数据库，设置 create_if_missing
        if !db_url.contains(":memory:") {
            let connect_options = SqliteConnectOptions::from_str(db_url)
                .map_err(|e| {
                    anyhow::anyhow!("Invalid SQLite connection string '{}': {}", db_url, e)
                })?
                .create_if_missing(true)
                .journal_mode(SqliteJournalMode::Wal); // 使用 WAL 模式提高性能
    ...
    async fn init_schema(&self) -> Result<()> {
        ...
        // 检查并添加 avatar_url 列
        let add_avatar_url = r#"
            ALTER TABLE sessions ADD COLUMN avatar_url TEXT
        "#;
        if let Err(_) = sqlx::query(add_avatar_url).execute(&self.pool).await {
            // 列可能已存在，忽略错误
        }
        ...
        sqlx::query(
            r#"
            CREATE INDEX IF NOT EXISTS idx_pending_messages_status_priority 
            ON pending_messages(status, priority DESC, created_at ASC)
            "#,
        )
        .execute(&self.pool)
        .await
        .context("Failed to create pending_messages index")?;
    ...
    fn extract_seq_from_message(message: &Message) -> Option<i64> {
        ...
        if seq_top > 0 {
            ...
            message.extra.get("seq").and_then(|v| v.parse::<i64>().ok())
        }
    }
...
impl StorageBackend for SqliteStorage {
    ...
    async fn save_message(&self, message: &Message) -> Result<()> {
        ...
        let extra_json =
            serde_json::to_string(&message.extra).context("Failed to serialize message extra")?;
    ...
    async fn batch_save_messages(&self, messages: &[Message]) -> Result<()> {
        ...
        let mut tx = self
            ...
            .begin()
            .await
            .context("Failed to begin transaction for batch save")?;
        ...
        for message in messages {
            ...
            .execute(&mut *tx)
            .await
            .context("Failed to save message in batch transaction")?;
        }
    ...
    async fn get_message(&self, message_id: &str) -> Result<Option<Message>> {
        ...
        if let Some(row) = row {
            ...
            let message =
                Message::decode(&content_bytes[..]).context("Failed to decode message")?;
            Ok(Some(message))
        } else {
            Ok(None)
        }
    ...
    async fn save_session(&self, session: &SessionSummary) -> Result<()> {
        ...
        sqlx::query(
            ...
            (session_id, session_type, business_type, last_message_id, last_message_time,
             ...
             metadata, server_cursor_ts, display_name, avatar_url, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            "#,
        )
        ...
        .bind(session.avatar_url.as_ref())
        .bind(updated_at)
        .execute(&self.pool)
        .await
    ...
    async fn get_session(&self, session_id: &str) -> Result<Option<SessionSummary>> {
        let row = sqlx::query(
            r#"
            SELECT session_id, session_type, business_type, display_name, avatar_url,
                   last_message_id, last_message_time, last_sender_id, 
                   last_message_type, last_content_type, unread_count, 
                   metadata, updated_at
        ...
        if let Some(row) = row {
            ...
            // 构建消息预览对象 - 只在SDK需要时进行详细解析
            let last_message = if let (
                Some(message_id),
                Some(sender_id),
                Some(message_type),
                Some(message_time),
            ) = (
                row.get::<Option<String>, _>("last_message_id"),
                row.get::<Option<String>, _>("last_sender_id"),
                row.get::<Option<i32>, _>("last_message_type"),
                row.get::<Option<i64>, _>("last_message_time"),
            ) {
                // 根据proto样式定义，MessagePreview的type字段直接使用i32
                Some(flare_proto::common::MessagePreview {
                    message_id,
                    sender_id,
                    r#type: message_type, // 直接使用i32类型的message_type
                    text: "".to_string(), // 可以从last_content_type或其他地方提取文本预览
                    time: Some(prost_types::Timestamp {
                        seconds: message_time / 1000,
                        nanos: ((message_time % 1000) * 1_000_000) as i32,
                    }),
                })
            } else {
                None
            ...
            Ok(Some(SessionSummary {
                ...
                last_message,
                unread_count: row.get::<i32, _>("unread_count") as u32,
                max_seq: 0, // 这些字段需要从sync_cursors表获取
                last_read_seq: 0,
                is_muted: false,  // 默认值
                is_pinned: false, // 默认值
    ...
    async fn save_sync_cursor(&self, session_id: &str, cursor: &SyncCursor) -> Result<()> {
        ...
        .bind(if recent_start_seq > 0 {
            Some(recent_start_seq)
        } else {
            None
        })
        .bind(if recent_end_seq > 0 {
            Some(recent_end_seq)
        } else {
            None
        })
    ...
    async fn get_sync_cursor(&self, session_id: &str) -> Result<Option<SyncCursor>> {
        ...
        if let Some(row) = row {
            ...
            let recent_sync_range =
                if let (Some(start), Some(end)) = (recent_start_seq, recent_end_seq) {
                    if start > 0 && end > 0 {
                        Some((start, end))
                    } else {
                        None
            ...
            Ok(Some(SyncCursor {
                ...
                recent_messages_synced: row
                    .try_get::<Option<i32>, _>("recent_messages_synced")
                    .ok()
                    .flatten()
                    .map(|v| v != 0)
                    .unwrap_or(false),
    ...
    async fn save_message_extension(
        ...
        let extension_json =
            serde_json::to_string(extension).context("Failed to serialize message extension")?;
        let updated_at = chrono::Utc::now().timestamp_millis();
    ...
    async fn save_session_extension(
        ...
        let extension_json =
            serde_json::to_string(extension).context("Failed to serialize session extension")?;
        let updated_at = chrono::Utc::now().timestamp_millis();
...
mod tests {
    ...
    async fn test_sqlite_storage_init() {
        let _storage = SqliteStorage::new(":memory:").await.unwrap();
        // 如果初始化成功，不会 panic
    }
