...
mod wasm_impl {
    ...
    use web_sys::{
        IdbDatabase, IdbFactory, IdbIndex, IdbKeyRange, IdbObjectStore, IdbOpenDbRequest,
    ...
    impl crate::infrastructure::storage::storage_trait::StorageSyncBounds for IndexedDBStorage {}
    ...
    impl IndexedDBStorage {
        ...
        pub async fn new(db_name: &str) -> Result<Self> {
            let window = web_sys::window().context("Failed to get window object")?;

            let idb_factory_opt = window
                .indexed_db()
                .map_err(|e| anyhow::anyhow!("Failed to get IndexedDB factory: {:?}", e))?;
            let idb_factory: IdbFactory =
                idb_factory_opt.ok_or_else(|| anyhow::anyhow!("IndexedDB not supported"))?;
                                ...
                                if let Ok(db) = result.dyn_into::<IdbDatabase>() {
                                    ...
                                    if let Ok(store) = messages_store {
                                        if old_version < 2 {
                                            ...
                                            if let Err(e) =
                                                store.create_index("session_id", "session_id", None)
                                            {
                                                tracing::warn!(error = ?e, "Failed to create session_id index (may already exist)");
                                            } else {
                                                tracing::info!(
                                                    "Created session_id index for messages store"
                                                );
                                            }
                                        }
                                    }
        ...
        fn deserialize_message(value: &JsValue) -> Result<Message> {
            ...
            Message::decode(&bytes[..]).context("Failed to decode message")
        }
    }
    ...
    impl StorageBackend for IndexedDBStorage {
        ...
        async fn batch_save_messages(&self, messages: &[Message]) -> Result<()> {
            ...
            for message in messages {
                ...
                store
                    .put_with_key(&JsValue::from_str(&message.id), &value)
                    .map_err(|e| anyhow::anyhow!("Failed to put message in batch: {:?}", e))?;
            }
        ...
        async fn get_message(&self, message_id: &str) -> Result<Option<Message>> {
            ...
            let result = Self::await_request(request)
                .await
        ...
        async fn get_max_seq(&self, session_id: &str) -> Result<Option<i64>> {
            ...
            let max_seq = messages
                .into_iter()
                .filter_map(|msg| msg.extra.get("seq").and_then(|v| v.parse::<i64>().ok()))
                .max();
        ...
        async fn delete_message(&self, message_id: &str) -> Result<()> {
            let store = self.get_object_store("messages", IdbTransactionMode::Readwrite)?;
            store
                .delete(&JsValue::from_str(message_id))
        ...
        async fn save_session(&self, session: &SessionSummary) -> Result<()> {
            ...
            store
                .put_with_key(&JsValue::from_str(&session.session_id), &value)
        ...
        async fn get_session(&self, session_id: &str) -> Result<Option<SessionSummary>> {
            ...
            let request = store
                .get(&JsValue::from_str(session_id))
        ...
        async fn get_sessions(&self, filter: SessionFilter) -> Result<Vec<SessionSummary>> {
            ...
            for i in 0..array.length() {
                ...
                match serde_wasm_bindgen::from_value::<SessionSummary>(value.clone()) {
                    Ok(session) => {
                        // 应用过滤条件
                        let matches = (filter.session_type.is_none()
                            || filter.session_type.as_ref() == Some(&session.session_type))
                            && (filter.business_type.is_none()
                                || filter.business_type.as_ref() == Some(&session.business_type))
        ...
        async fn update_session(&self, session_id: &str, updates: SessionUpdate) -> Result<()> {
            ...
            if let Some(ref last_msg) = updates.last_message {
                ...
                session.last_message = Some(flare_proto::common::MessagePreview {
                    message_id: last_msg.message_id.clone(),
                    sender_id: last_msg.sender_id.clone().unwrap_or_default(),
                    r#type: match last_msg.message_type {
                        1 => flare_proto::common::MessageType::MessageTypeText,
                        2 => flare_proto::common::MessageType::MessageTypeImage,
                        3 => flare_proto::common::MessageType::MessageTypeAudio,
                        4 => flare_proto::common::MessageType::MessageTypeVideo,
                        5 => flare_proto::common::MessageType::MessageTypeFile,
                        6 => flare_proto::common::MessageType::MessageTypeLocation,
                        7 => flare_proto::common::MessageType::MessageTypeNotification,
                        _ => flare_proto::common::MessageType::MessageTypeCustom,
                    },
                    text: if last_msg.content_type == "text" {
                        "".to_string()
                    } else {
                        last_msg.content_type.clone()
                    },
                    time: Some(Timestamp {
                        seconds: last_msg.message_time / 1000,
                        nanos: ((last_msg.message_time % 1000) * 1_000_000) as i32,
                    }),
                });
            }
        ...
        async fn delete_session(&self, session_id: &str) -> Result<()> {
            let store = self.get_object_store("sessions", IdbTransactionMode::Readwrite)?;
            store
                .delete(&JsValue::from_str(session_id))
        ...
        async fn save_sync_cursor(&self, session_id: &str, cursor: &SyncCursor) -> Result<()> {
            ...
            store
                .put_with_key(&JsValue::from_str(session_id), &value)
        ...
        async fn save_message_state(
            ...
            store
                .put_with_key(&JsValue::from_str(&key), &value)
