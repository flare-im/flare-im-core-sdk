...
use crate::infrastructure::storage::storage_trait::{StorageBackend, SessionFilter, SessionUpdate, LastMessageUpdate, MessageState, StorageSyncBounds};
use anyhow::Result;
    ...
    async fn update_session(&self, session_id: &str, updates: SessionUpdate) -> Result<()> {
        if let Some(s) = self.sessions.borrow_mut().get_mut(session_id) {
            ...
            if let Some(last) = updates.last_message {
                ...
                s.last_message = Some(flare_proto::common::MessagePreview {
                    message_id: last.message_id.clone(),
                    sender_id: last.sender_id.clone().unwrap_or_default(),
                    r#type: match last.message_type {
                    ...
                    text: if last.content_type == "text" { "".to_string() } else { last.content_type.clone() },
                    time: Some(Timestamp {
                        seconds: last.message_time / 1000,
                        nanos: ((last.message_time % 1000) * 1_000_000) as i32,
                    }),
                });
            }
        }
