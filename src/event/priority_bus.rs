        ...
        match sender.send(event) {
            ...
            Err(broadcast::error::SendError(_)) => {
                ...
                if priority == EventPriority::Critical {
                    ...
                    // 非关键事件直接丢弃
                    tracing::debug!("Event dropped due to channel full (priority: {:?})", priority);
                }
            }
        }
    }
    ...
    fn infer_priority(event: &Event) -> EventPriority {
        match event {
            ...
            Event::Message(me) => match me {
                crate::event::MessageEvent::MessageReceived { .. } => EventPriority::High,
                crate::event::MessageEvent::MessageSent { .. } => EventPriority::High,
                crate::event::MessageEvent::MessageFailed { .. } => EventPriority::High,
