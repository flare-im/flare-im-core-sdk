...
use crate::model::{Message, SessionSummary};
...
impl CachedStorage {
    ...
    fn evict_if_needed(&self) {
        ...
        if self.message_cache.len() > self.max_cache_size {
            ...
            // 收集最旧的 key（按过期时间排序）
            let mut items: Vec<_> = self.message_cache
                .iter()
        ...
        if self.session_cache.len() > self.max_cache_size {
            ...
            let mut items: Vec<_> = self.session_cache
                .iter()
                .map(|entry| (entry.key().clone(), entry.value().expires_at))
                .collect();
...
impl crate::storage::storage_trait::StorageSyncBounds for CachedStorage {}
...
impl StorageBackend for CachedStorage {
    ...
    async fn save_session(&self, session: &SessionSummary) -> Result<()> {
        ...
        self.session_cache.insert(session.session_id.clone(), cached);
    ...
    async fn get_sessions(&self, filter: crate::storage::SessionFilter) -> Result<Vec<SessionSummary>> {
        // 会话列表查询不缓存（变化频繁）
    ...
    async fn update_session(
        &self,
        session_id: &str,
        updates: crate::storage::SessionUpdate,
    ) -> Result<()> {
        // 更新底层存储
    ...
    async fn save_sync_cursor(&self, session_id: &str, cursor: &crate::model::SyncCursor) -> Result<()> {
        self.storage.save_sync_cursor(session_id, cursor).await
    }
    
    async fn get_sync_cursor(&self, session_id: &str) -> Result<Option<crate::model::SyncCursor>> {
    ...
    async fn save_message_state(
        ...
        user_id: &str,
        message_id: &str,
        state: crate::storage::MessageState,
    ) -> Result<()> {
    ...
    async fn save_message_extension(
        &self,
        message_id: &str,
        extension: &crate::model::MessageExtension,
    ) -> Result<()> {
        self.storage.save_message_extension(message_id, extension).await
    }
    ...
    async fn save_session_extension(
        &self,
        session_id: &str,
        extension: &crate::model::SessionExtension,
    ) -> Result<()> {
