...
impl CryptoService for NoopCrypto {
    fn encrypt(&self, data: &[u8]) -> Result<Vec<u8>> { Ok(data.to_vec()) }
    fn decrypt(&self, data: &[u8]) -> Result<Vec<u8>> { Ok(data.to_vec()) }
}
...
impl CryptoService for AesCrypto {
    ...
    fn decrypt(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let pt = self.cipher.decrypt(Nonce::from_slice(nonce_bytes), ct)
