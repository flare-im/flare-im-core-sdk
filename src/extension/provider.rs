...
impl ExtensionManager {
    ...
    pub async fn enrich_message(
        &self,
        message: &mut crate::model::ExtendedMessage,
    ) -> Result<()> {
        ...
        for provider in providers.iter() {
            if let Some(ext) = provider.get_user_extension(user_id).await? {
                message.extension.sender_avatar = ext.avatar.clone().or(message.extension.sender_avatar.clone());
                message.extension.sender_name = ext.name.clone().or(message.extension.sender_name.clone());
    ...
    pub async fn enrich_session(
        &self,
        session: &mut crate::model::ExtendedSessionSummary,
    ) -> Result<()> {
        ...
        for provider in providers.iter() {
            if let Some(ext) = provider.get_session_extension(session_id).await? {
                ...
                session.extension.display_name = ext.display_name.clone().or(session.extension.display_name.clone());
                session.extension.is_pinned = ext.is_pinned;
                session.extension.is_muted = ext.is_muted;
    ...
    pub async fn batch_enrich_messages(
        &self,
        messages: &mut [crate::model::ExtendedMessage],
    ) -> Result<()> {
        ...
        if let Some(cache) = &self.cache {
            ...
            let cache_tasks: Vec<_> = user_ids.iter()
                .map(|user_id| {
                    let cache = Arc::clone(cache);
                    let user_id = user_id.clone();
                    tokio_spawn(async move {
                        cache.get_user_extension(&user_id).await
                            .ok()
                            .flatten()
                            .map(|ext| (user_id, ext))
                    })
        ...
        let mut provider_extensions = std::collections::HashMap::with_capacity(uncached_user_ids.len());
        ...
        // 4. 更新缓存（优化：并行更新）
        if let Some(cache) = &self.cache {
            let cache_tasks: Vec<_> = provider_extensions.iter()
                .map(|(user_id, ext)| {
                    let cache = Arc::clone(cache);
                    let user_id = user_id.clone();
                    let ext = ext.clone();
    ...
    pub async fn batch_enrich_sessions(
        &self,
        sessions: &mut [crate::model::ExtendedSessionSummary],
    ) -> Result<()> {
        // 类似逻辑...
